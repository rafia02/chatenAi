...
//   useEffect(() => {
//     fetch(`http://localhost:5000/sub?email=${user?.email}`)
//       .then((res) => res.json())
//       .then((data) => {
//         console.log("..................................", data);
...
// console.log(allsub)
  ...
  const sendMessage = async () => {
    ...
    console.log("aaaaaaaaaaaaaaaaaaaaaaaaa" ,response.data.candidates[0].content.parts[0].text)
...
//     queryKey: ['sub'],
//     queryFn: async () => {
//         const res = await fetch(`http://localhost:5000/datas?email=${user?.email}??sub=${show}`)
//         const data = await res.json()
//         return data
...
// })
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/datas?email=${user?.email}&&sub=${show}`)
      .then((res) => res.json())
      .then((data) => {
        console.log("..................................", data);
        refetch()
        ...
        >
          ...
          <div className="p-4 h-80 capitalize flex flex-col gap-3 overflow-y-auto text-[#8386c4] scrollbar-hide">
            {allsub.map((element, index) => (
                ...
                <button onClick={()=>setShow(element.main)} className={`cursor-pointer text-start  px-5 rounded py-1 ${show === element.main ? "bg-gray-700" : ""}`} key={index}>{element.main}</button>
                      ...
                      <div className="box2 flex justify-items-end mb-10 self-end  bg-gradient-to-r from-[#282953] via-[#1a1c3a] to-[#14152c] rounded-2xl duration-500 px-10 py-6">
                        <div className="flex items-center leading-loose gap-7">
                          ...
                          <div>
                            ...
                            <p className="text-[#8386c4] mt-2"><Typewriter
              words={[msg.sub]}
              loop:true
              cursor
              cursorStyle="|"
